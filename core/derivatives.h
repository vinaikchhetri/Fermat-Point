...
Interval operator* (const vector < double > &row, const vector < Interval > &col);
...
vector < vector < Interval >> operator* (const vector < vector < double >> &inv, const vector < vector < Interval >> &hes);
vector < vector < Interval >> operator* (const vector < vector <Interval >> &inv, const vector < vector <Interval >> &hes);
vector < vector < Interval >> operator+ (const vector < vector <Interval >> &M1, const vector < vector <Interval >> &M2);

vector < vector < double >> operator* (const vector < vector < double >> &inv, const vector < vector < double >> &hes);
double operator* (const vector < double > &row, const vector < double > &col);
...
inline Box Newton (const Box &B, const vector < Point > &foci){
  ...
  cout<<"4.real_hessian_mat"<<"\n";
  //cout<<real_hessian_mat.size()<<"\n";
  //cout<<real_hessian_mat[0].size()<<"\n";

  //K transposed.
  inv_transposed = inverse (real_hessian_mat);
  cout<<"5.inv_transposed"<<"\n";
  // cout<<inv_transposed<<"\n";

  //rough
  // vector < vector < double >> iden = init2(2);

  // vector < vector < Interval >> iiden = mat2Interval(iden);
  // vector < vector < Interval >> iiden2 = mat2Interval(iden);

  // iiden = iiden+iiden;
  // iiden2 = iiden2*iiden2;
  // cout<<"iiden"<<"\n";
  // cout<<iiden<<"\n";
  // cout<<"iiden2"<<"\n";
  // cout<<iiden2<<"\n";
  // vector<vector<Interval>> bn1 = {{Interval(1.2,1.2),Interval(3.3,3.3)},{Interval(4.4,4.4),Interval(5.5,5.5)}};
  // vector<vector<Interval>> bn2 = {{Interval(1,1),Interval(2,2)},{Interval(3,3),Interval(4,4)}};
  // vector<vector<double>> bn3 = {{1.2,3.3},{4.4,5.5}};
  // cout<<"bn1"<<bn1;
  // cout<<"bn2"<<bn2;
  // cout<<"bn3"<<bn3;
  // bn2 = transposeInt(bn2);

  // cout<<"transposeInt(bn2)"<<bn2;
  // vector<vector<Interval>> bn4 = bn1*bn2; 
  // cout<<"int*int"<<bn4;
  
  // bn4 = bn3*bn2; 
  // cout<<"double*int"<<bn4;

  // vector<vector<Interval>> c = bn1*transposeInt(box_hessian);
  // cout<<"c"<<c;
  //

  //K * J_f(B)
  vector < vector < double >> invR = transpose(inv_transposed);
  cout<<"6.invR"<<"\n";
  //vector < vector < Interval >> invInt=  mat2Interval (invR);
  //cout<<"7.invInt"<<"\n";
  vector < vector < Interval >> box_hessianT = transposeInt(box_hessian);
  cout<<"7.box_hessianT"<<"\n";
  // cout<<"invInt"<<invInt;
  //cout<<"box_hessianT"<<box_hessianT;
  //K_Jf = invInt * box_hessianT;
  //cout<<"9.o-K_Jf"<<"\n";
  //cout<<K_Jf;

  //vector < vector < double >> I1 = init2 (100);
  //vector < vector < Interval >> I2 = mat2Interval(I1);
  //I2*I2;
 
  //cout<<invR<<"\n";
  //cout<<invR[0][0]<<"\n";

  //cout<<box_hessianT.size()<<"\n";
  //cout<<box_hessianT[1].size()<<"\n";
  //vector<vector<double>> v = invR * real_hessian_mat;
  //cout<<"donnnne";
  K_Jf = invR * box_hessianT;
  cout<<"9.K_Jf"<<"\n";
  //cout<<K_Jf;
...
inline vector < vector < Interval >>hes_eval (const Box & B, const Point & a){
  ...
  double small = den.min;  
  double big = den.max;
  small = small.approx(120,120);
  big = big.approx(120,120);
  den_sq.min = small * small;
  den_sq.max = big * big;
  
  //Computing ||B-a||^3 for the denominator for each of the entries.
  Interval den_cub (pow (small, 3), pow (big, 3));
...
inline vector < vector < double >>real_hessian (const vector < Point > &foci, const Point & p){
  ...
  for (int a = 0; a < foci.size (); a++){
    diff = p - foci[a];
    den = normP (diff);
    den =den.approx(120,120);
    norm_sq = pow (den, 2);
    den = pow (den, 3);

    for (int i = 0; i < d; i++){
       sub = p.point[i] - foci[a].point[i];
       sub = sub.approx(120,120);
      for (int j = 0; j < d; j++){
        if (i == j){
          computation = norm_sq - pow (sub, 2);
          mat[i][j] += computation / den;
        }
        else
        {
          computation = Expr(-1) * (sub) * (p.point[j] - foci[a].point[j]);
          mat[i][j] += computation / den;
        }
      }
    }
...
inline double operator* (const vector < double > &row, const vector < double > &col){
  double interval;
  double ii;
  // Interval i1;
  // double i2;
...
inline Interval operator* (const vector < double > &row, const vector < Interval > &col){
  Interval interval;
  Interval ii;
  // Interval i1;
  // double i2;
...
inline vector < vector < Interval >> operator* (const vector < vector < double >> &inv, const vector < vector < Interval >> &hes){
  Interval interval;
  vector < Interval > row;
  vector < vector < Interval >> ret;
...
//multiplication of matrices is defined as row to row. not row to col.
inline vector < vector < double >> operator* (const vector < vector < double >> &inv, const vector < vector < double >> &hes){
  double interval;
  vector < double > row;
  vector < vector < double >> ret;
