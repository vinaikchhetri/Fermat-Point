...
int main(){
  ...
  ofstream input ("3d-unit-circle.txt" , ios::app);
  for (int i=0; i<nrolls; i++) {
    double number1 = distribution(generator);
    double number2 = distribution(generator);
    double number3 = distribution(generator);
    if ((number1 == 0.0) && (number2 == 0.0) && (number3 == 0.0)) i=i-1;
    deno = (number1*number1) + (number2*number2) + (number3*number3);
    deno = sqrt(deno);
    deno = 1/deno;
    number1 = number1*deno;
    number2 = number2*deno;
    number3 = number3*deno;
